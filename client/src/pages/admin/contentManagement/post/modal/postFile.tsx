...
import { add, getById, update } from 'src/state/slices/contentManagement/postFileSlice';
...
import { getAllByParams, remove } from 'src/state/slices/contentManagement/postFileSlice';
...
const PostFile = ({id , permissions, locale}: FormProps) => {
...
const [postFile, setPostFile] = useState<PostFileDTO | null>(null);
const { postFiles, totalCount, isLoading } = useSelector((state: any) => state?.postFile?.postFiles ? state?.postFile : { postFiles: [], totalCount: 0, isLoading: false });
...
const getItemById = async (id: string | number) => {
  const postFileDTO: PostFileDTO = await dispatch(getById(id)).unwrap();
  await setFormData(postFileDTO);
  setPostFile(postFileDTO);
}

const setFormData = async (postFileDTO: PostFileDTO | null) =>{
  if (postFileDTO) {
    await formik.setValues({
        name: postFileDTO.name,
        file: postFileDTO.file,
        description: postFileDTO.description,
        priority: postFileDTO.priority
      } as initialValuesType);
      setFile(postFileDTO.file);
      setFileExtension(postFileDTO.fileExtension);
  }
  else{
   clearFormData(); 
  }
...
const initialValues: initialValuesType = {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      if(formik.isValid){
        if (isUpdate) {
          const updatePostFileDTO: UpdatePostFileDTO = {
            id: rowId!,
            postId: id!,
            name: values.name,
          ...
          const result = await dispatch(update(updatePostFileDTO)).unwrap();
          if (result) {
            getGridData();
          }
        } else {
          const addPostFileDTO: AddPostFileDTO = {
            postId: id!,
            name: values.name,
            description: values.description,
            file: file,
          ...
          const result = await dispatch(add(addPostFileDTO)).unwrap();
          if (result) {
            getGridData();
            await clearFormData();
          }         
  ...
  const getGridData = () => {
    ...
    const listPostFileByParams: ListPostFileByParamsDTO = {
      postId: id!,
      gridParameter: gridparameter
    };
    dispatch(getAllByParams(listPostFileByParams))
  }
