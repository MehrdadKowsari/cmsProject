...
import { getAllByParams, remove, toggleActive } from 'src/state/slices/contentManagement/menuSlice';
...
const Page = ({ Component, pageProps }: AppProps) => {
  ...
  const { menus, totalCount, isLoading } = useSelector((state: any) => state?.menu?.menus ? state?.menu : { menus: [], totalCount: 0, isLoading: false });
  ...
  const getRolePagePermissions = async () => {
    const permissions: PermissionDTO[] = await dispatch(getAllByPageId(PageTypeEnum.Menu)).unwrap();
  ...
  return (
    <>
      {hasViewPermission && <Card>
        <CardHeader
          title={t('menu', CommonMessage.Menu)}
        ...
        <CardContent>
          ...
          <CustomDialog
            title={t('menuItem', CommonMessage.MenuItem)}
