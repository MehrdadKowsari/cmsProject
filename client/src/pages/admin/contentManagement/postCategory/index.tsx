...
import { getAllByParams, remove, toggleActive } from "src/state/slices/contentManagement/postCategorySlice";
...
const Page = ({ Component, pageProps }: AppProps) => {
  ...
  const { postCategories, totalCount, isLoading } = useSelector((state: any) => state?.postCategory?.postCategories ? state?.postCategory : { postCategories: [], totalCount: 0, isLoading: false });
  ...
  const getRolePagePermissions = async () => {
    const permissions: PermissionDTO[] = await dispatch(getAllByPageId(PageTypeEnum.PostCategory)).unwrap();
  ...
  return (
    <>
      {hasViewPermission && <Card>
        <CardHeader
          title={t('postCategory', CommonMessage.PostCategory)}
