...
import { getAllByParams, remove, toggleActive } from 'src/state/slices/contentManagement/gallerySlice';
...
const Page = ({ Component, pageProps }: AppProps) => {
  ...
  const getRolePagePermissions = async () => {
    const permissions: PermissionDTO[] = await dispatch(getAllByPageId(PageTypeEnum.Gallery)).unwrap();
  ...
  return (
    <>
      {hasViewPermission && <Card>
        <CardHeader
          title={t('gallery', CommonMessage.Gallery)}
        ...
        <CardContent>
          ...
          <CustomDialog
            title={t('galleryFile', CommonMessage.GalleryFile)}
