...
const API_URL: string = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/slider`;

export const add = createAsyncThunk(
  "sliders/add", 
  async (slider: AddSliderDTO, { rejectWithValue }) => {
    try {
      const { data } = await axios.post(`${API_URL}/add`, slider)
      return data?.result;
    } catch (err) {
...
export const update = createAsyncThunk(
  "sliders/update", 
  async (slider: UpdateSliderDTO, { rejectWithValue }) => {
    try {
      const { data } = await axios.post(`${API_URL}/update`, slider)
      return data?.result;
    } catch (err) {
        const error= err as AxiosError;
        return rejectWithValue(error.message);
...
interface SliderState extends IntialState {
  sliders: SliderDTO[] | null,
  slider: SliderDTO | null
}
