...
export const add = createAsyncThunk(
  "sliderItems/add", 
  async (sliderItem: AddSliderItemDTO, { rejectWithValue }) => {
    try {
      const { data } = await axios.post(`${API_URL}/add`, sliderItem)
      return data?.result;
    } catch (err) {
...
export const update = createAsyncThunk(
  "sliderItems/update", 
  async (sliderItem: UpdateSliderItemDTO, { rejectWithValue }) => {
    try {
      const { data } = await axios.post(`${API_URL}/update`, sliderItem)
      return data?.result;
    } catch (err) {
        const error= err as AxiosError;
        return rejectWithValue(error.message);
...
const sliderItemSlice = createSlice({
    name: 'sliderItems',
    initialState,
    reducers: {},
    extraReducers: (builder) => {
        builder
          ...
          .addCase(getAllByParams.fulfilled, (state, { payload }) => {
            state.sliderItems = payload.rows;
