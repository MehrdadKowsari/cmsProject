...
const API_URL: string = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/page`;
...
export const toggleHidden = createAsyncThunk(
  ...
  async (id: string | number, { rejectWithValue }) => {
      try {
          const { data } = await axios.post(`${API_URL}/toggleHidden`, id);
          return data?.result;
      } catch (err) {
          const error= err as AxiosError;
          return rejectWithValue(error.message);
...
const pageSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
          ...
          .addCase(toggleHidden.pending, (state) => {
            state.isLoading = true;
            state.hasError = false;
          })
          .addCase(toggleHidden.fulfilled, (state) => {
            state.isLoading = false;
            state.hasError = false;
          })
          .addCase(toggleHidden.rejected, (state, { payload }) => {
            state.hasError = true;
            state.isLoading = false;
            state.error = <string>payload;
          })
