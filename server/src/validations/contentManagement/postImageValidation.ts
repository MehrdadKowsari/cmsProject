...
const addValidation = (req: Request, res: Response, next: NextFunction) => {
     const addValidationSchema = joi.object({
          ...
          imageSavePath: joi.string().optional().allow(null, '').max(AppConstant.FileSavePathMaxLength).label('File Save Path')
          .messages({ 
               "string.max": req.t('maxLenghtForFileSavePathIsN', `{{#label}} max lenght is {{#limit}}`, {n: `{{#limit}}`})}),          
          imageExtension: joi.string().optional().allow(null, '').max(AppConstant.FileExtensionMaxLength).label('File Extension')
          .messages({ 
