...
const addValidation = (req: Request, res: Response, next: NextFunction) => {
     const addValidationSchema = joi.object({
          galleryId: joi.string().required().label('GalleryId')
          .messages({ 
               "strting.base": req.t('galleryIdIsRequired', `{{#label}} is required`),
               "string.empty": req.t('galleryIdIsRequired', `{{#label}} is required`),
               "strting.required": req.t('galleryIdIsRequired', `{{#label}} is required`)}),
