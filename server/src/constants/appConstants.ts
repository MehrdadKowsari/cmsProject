export default class AppConstant {
    public static readonly PageSize: number = 10;
    public static readonly FirstnameMinLenght:number = 2;
    public static readonly FirstnameMaxLenght:number = 50;
    public static readonly LastnameMinLenght:number = 2;
    public static readonly LastnameMaxLenght:number = 50;
    public static readonly UsernameMinLenght:number = 3;
    public static readonly UsernameMaxLenght:number = 50;
    public static readonly PasswordMinLenght:number = 6;
    public static readonly NameMaxLenght:number = 200;
    public static readonly TitleMaxLenght:number = 1000;
    public static readonly DescriptionMaxLenght:number = 500;
    public static readonly AllowedFileExtensionMaxLength:number = 200;
    public static readonly SlugUrlMaxLength:number = 2000;
    public static readonly UrlMaxLength:number = 2000;
    public static readonly EmailMaxLength:number = 200;
    public static readonly FullNameMaxLength:number = 200;
    public static readonly WebsiteMaxLength:number = 200;
    public static readonly IPMaxLength:number = 200;
    public static readonly FileSavePathMaxLength:number = 5000;
    public static readonly FileExtensionMaxLength:number = 100;
    public static readonly SectionNameMaxLength:number = 200;
    public static readonly Locale:number = 10;
}